...
export const navItems = [
  ...
  },
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden>
        <path d="M4 10l3 3 6-7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  ...
  },
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden>
        ...
        <path d="M9 1v2M9 15v2M1 9h2M15 9h2M3.22 3.22l1.42 1.42M13.36 13.36l1.42 1.42M3.22 14.78l1.42-1.42M13.36 4.64l1.42-1.42" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
      </svg>
    ),
  },
];
...
function Logo({ onClick }) {
  return (
    <Link to="/" onClick={onClick} className="flex items-center gap-2.5 pl-2 mb-9 no-underline group">
...
function NavLink({ to, label, icon, onNavigate }) {
  ...
  return (
    <Link to={to} onClick={onNavigate} className="no-underline block">
      <div className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-[14px] transition-all duration-150 cursor-pointer ${
        active ? "bg-brand-bg text-brand font-semibold" : "text-[#6b6b8a] font-medium hover:bg-[#f7f5ff] hover:text-brand"
      }`}>
        <span className="shrink-0">{icon}</span>
        {label}
        {active && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-brand shrink-0" />}
      </div>
    </Link>
  );
}

// ── User card — now reads real auth data instead of hardcoded "Nikita" ────────
function UserCard({ onNavigate }) {
  const { user } = useAuth();
  const displayName = user?.name || "Guest";
  const initial = (user?.name || "U").charAt(0).toUpperCase();
  const plan = user?.plan ? `${user.plan.charAt(0).toUpperCase()}${user.plan.slice(1)} plan` : "Free plan";
    ...
    <Link to="/profile" onClick={onNavigate} className="no-underline mb-3">
      <div className="flex items-center gap-2.5 p-3 rounded-[14px] bg-[#f7f5ff] border border-[#ede9f5] hover:border-violet-200 hover:bg-brand-bg/60 transition-colors duration-150">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[11px] text-brand-muted">{plan}</div>
        </div>
...
export function SideBarDesktop({ className = "" }) {
  return (
    <aside className={`hidden md:flex w-[220px] shrink-0 flex-col bg-white border-r border-border px-4 py-7 min-h-[calc(100vh-64px)] sticky top-16 ${className}`}>
      <SidebarContent />
    </aside>
  );
}

/** Mobile drawer toggle button — exported so NavBar can render it */
...
/** Full sidebar with mobile drawer */
