...
// Ordered from most preferred → least preferred.
// gemini-2.5-flash is powerful but frequently 503s under load.
// gemini-1.5-flash is stable and fast — best default.
// gemini-1.5-flash-8b is the lightest fallback before we serve mock data.
const MODEL_CHAIN = [
  "gemini-1.5-flash",
  "gemini-1.5-flash-8b",
  "gemini-2.5-flash",   // kept as last real attempt — it works when not overloaded
];

const genAI = env.geminiApiKey ? new GoogleGenerativeAI(env.geminiApiKey) : null;

// Build a model instance on demand so we can swap during fallback
...
// Only retries on 503 / 429 (overload / rate-limit) errors.
async function withRetry(fn, maxAttempts = 2, baseDelayMs = 800) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const delay = baseDelayMs * Math.pow(2, attempt - 1); // 800ms, 1600ms …
      console.warn(
        `[Gemini] Attempt ${attempt} failed (${msg.slice(0, 80)}). Retrying in ${delay}ms…`
      );
...
// ─── Mock data (returned when ALL models fail) ────────────────────────────────
export const MOCK_PACK = {
  theory:
    "This topic is a core interview concept. Focus on explaining the core idea, real-world use cases, and trade-offs clearly.",
  interviewQuestions: [
  ...
  mcqs: [
    {
      id: 1,
      question: "Which statement best describes this concept?",
      options: ["It improves performance", "It reduces readability", "It is always synchronous", "It only works in JavaScript"],
      answer: 0,
    },
    {
      id: 2,
      question: "What is a common use case?",
      options: ["Styling web pages", "Managing application state", "Writing database queries", "Configuring servers"],
      answer: 1,
    },
  ],
  codingProblems: [
    {
      ...
      difficulty: "Easy",
      description: "Write a function that demonstrates your understanding of this concept.",
...
export function buildStudyPackPrompt(topic) {
  return `You are an expert interview coach. For the topic "${topic}", generate a JSON response with exactly this structure:
{
  ...
  "codingProblems": [
    {"id": 1, "title": "...", "difficulty": "Easy", "description": "...", "code": "// solution", "tag": "#tag"},
    {"id": 2, "title": "...", "difficulty": "Medium", "description": "...", "code": "// solution", "tag": "#tag"}
  ],
  "hrQuestions": [
    {"id": 1, "question": "...", "answer": "...", "tip": "..."},
    {"id": 2, "question": "...", "answer": "...", "tip": "..."},
...
export async function generateText(prompt) {
  ...
  for (const modelName of MODEL_CHAIN) {
    ...
    try {
      ...
      console.log("[Gemini] Response snippet:", rawText.slice(0, 300));
      return rawText.replace(/```json|```/g, "").trim();
    } catch (err) {
      const msg = err?.message || "";
      console.warn(`[Gemini] ✗ Model ${modelName} failed: ${msg.slice(0, 120)}`);

      // If this is the last model in the chain, throw a clean error
...
// ─── Main export used by ai.controller.js ────────────────────────────────────
export async function generateInterviewContent(topic) {
  ...
  try {
    const prompt = buildStudyPackPrompt(topic);
    const text = await generateText(prompt);
    const pack = parseStudyPack(text);
    return { ...pack, _source: "gemini" };
  } catch (err) {
    // If all models failed and it's a temporary overload, return mock as graceful fallback
    const isOverload =
    ...
    // For auth errors, bad API key, etc. — surface the clean message
    throw err;
  }
}